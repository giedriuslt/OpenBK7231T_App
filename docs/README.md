...
| [Drivers](https://github.com/openshwprojects/OpenBK7231T_App/blob/main/docs/drivers.md) (96 total) | Drivers allows you to control certain peripherals or enable certain features that are off by default. |
...
| [Console/Script commands](https://github.com/openshwprojects/OpenBK7231T_App/blob/main/docs/commands.md) (483 total) | There are multiple console commands that allow you to automate your devices. Commands can be entered manually in command line, can be send by HTTP (just like in Tasmota), can be send by MQTT and also can be scripted. |
...
| [Console/Script commands [Extended Edition]](https://github.com/openshwprojects/OpenBK7231T_App/blob/main/docs/commands-extended.md) (483 total) | More details on commands. |
| [Events](https://github.com/openshwprojects/OpenBK7231T_App/blob/main/docs/events.md) (46 total) | Events are named triggers raised by the firmware (e.g. button presses, channel changes, network/MQTT/NTP state, Tuya DP updates, IR receive decodes, power-monitor readings). Use addEventHandler / addEventHandler2 / addEventHandler3 to run console commands when an event fires; handlers can optionally match on the event's argument (integer or string). Event availability depends on which features/drivers are enabled in the build. |
